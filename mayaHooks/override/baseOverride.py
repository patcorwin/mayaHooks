...
def buildCallStr(indent, func, args):

    s = func.__module__ + '.' + func.__name__ + '(' + ', '.join( [autoQ(name) for name in args.split()] ) + ')'
...
def baseOverride(target, source):
    ...
    folder = os.path.dirname(os.path.dirname(__file__)).replace('\\', '/') + '/' + str(util.version())
